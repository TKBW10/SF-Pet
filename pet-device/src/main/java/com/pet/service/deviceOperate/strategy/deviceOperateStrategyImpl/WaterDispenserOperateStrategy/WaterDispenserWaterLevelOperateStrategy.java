...
@Component
public class WaterDispenserWaterLevelOperateStrategy implements DeviceOperateStrategy {

    private final DeviceWaterDispenserService deviceWaterDispenserService;

    public WaterDispenserWaterLevelOperateStrategy(DeviceWaterDispenserService deviceWaterDispenserService) {
    ...
    public boolean support(String deviceType, String operation) {
        return DeviceTypeConstants.WATER_DISPENSER.equals(deviceType)
                && OperateConstants.WATER_LEVEL.equals(operation);
    }

    @Override
    public Result<String> execute(OperateDto operateDto) {
        return deviceWaterDispenserService.operateWaterLevel(operateDto);
    }
}