...
@Component
public class AirPurifierFilterOperateStrategy implements DeviceOperateStrategy {

    private final DeviceAirPurifierService deviceAirPurifierService;

    public AirPurifierFilterOperateStrategy(DeviceAirPurifierService deviceAirPurifierService) {
    ...
    public Result<String> execute(OperateDto operateDto) {
        return deviceAirPurifierService.operateFilter(operateDto);
    }
}