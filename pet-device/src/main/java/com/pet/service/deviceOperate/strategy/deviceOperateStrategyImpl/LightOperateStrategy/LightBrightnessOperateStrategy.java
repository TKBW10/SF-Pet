...
@Component
public class LightBrightnessOperateStrategy implements DeviceOperateStrategy {

    private final DeviceLightService deviceLightService;

    public LightBrightnessOperateStrategy(DeviceLightService deviceLightService) {
    ...
    public boolean support(String deviceType, String operation) {
        return DeviceTypeConstants.LIGHT.equals(deviceType)
                && OperateConstants.BRIGHTNESS.equals(operation);
    }

    @Override
    public Result<String> execute(OperateDto operateDto) {
        return deviceLightService.operateBrightness(operateDto);
    }
}