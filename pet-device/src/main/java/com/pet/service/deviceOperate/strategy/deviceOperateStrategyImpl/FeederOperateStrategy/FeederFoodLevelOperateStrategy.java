...
@Component
public class FeederFoodLevelOperateStrategy implements DeviceOperateStrategy {

    private final DeviceFeederService deviceFeederService;

    public FeederFoodLevelOperateStrategy(DeviceFeederService deviceFeederService) {
    ...
    public boolean support(String deviceType, String operation) {
        return DeviceTypeConstants.FEEDER.equals(deviceType)
                && OperateConstants.FOOD_LEVEL.equals(operation);
    }

    @Override
    public Result<String> execute(OperateDto operateDto) {
        return deviceFeederService.operateFoodLevel(operateDto);
    }
}